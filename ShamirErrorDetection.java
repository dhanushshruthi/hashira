...
public class ShamirErrorDetection {
    ...
    public static void findWrongShare(Map<String, Object> data) {
        ...
        if (mostSuspicious != -1 && highestErrorRate > 50) {
            ...
            List<Integer> suspiciousShares = new ArrayList<>();
            for (Map.Entry<Integer, ShareAnalysis> entry : shareAnalysis.entrySet()) {
                if (entry.getValue().getErrorRate() > 30) {
                    suspiciousShares.add(entry.getKey());
                }
            }
            
            if (!suspiciousShares.isEmpty()) {
                System.out.println("Suspicious shares (high error rate):");
                for (int shareId : suspiciousShares) {
                    Share suspShare = shareMap.get(shareId);
                    System.out.printf("  Share %d value: %s\n", shareId, suspShare.decimalValue);
                }
            } else {
                System.out.println("No clearly suspicious shares found");
            }
